// LatticeLoader do React Bits (https://reactbits.dev), variante TypeScript + CSS.
// Adição local: o último símbolo (check/cruz) virou estado e o aviso de leitor de tela é escrito
// direto no elemento, para passar nas regras de lint do React (sem mudar o comportamento).
...
const LatticeLoader: React.FC<LatticeLoaderProps> = ({
  ...
  // Guarda o último símbolo (check ou cruz) enquanto volta a "working", para ele não trocar no fade
  const [mark, setMark] = useState<'done' | 'error'>(status === 'error' ? 'error' : 'done');
  ...
  return (
    ...
    >
      ...
      <span ref={srRef} className="lattice-loader__sr">{`${label}, in progress`}</span>
    </span>
  );
};
