// ElectricLogo do React Bits (https://reactbits.dev), variante TypeScript + CSS.
...
export interface ElectricLogoProps {
  ...
  cursorRadius?: number;
  theme?: 'dark' | 'light';
  /** Adição local: duração, em segundos, da transição entre duas formas (o original é fixo em 1.6). */
  morphDuration?: number;
  /** Adição local: resolução (px) usada para contornar a imagem. Menor = contorno mais rápido de calcular. */
  raster?: number;
  /** Adição local: limite de densidade de pixels do canvas (o original usa até 2). */
  maxDpr?: number;
  /** Adição local: imagens a contornar com antecedência, para a troca de src não travar a tela. */
  preload?: string[];
  className?: string;
  style?: CSSProperties;
}

type Settings = Required<Omit<ElectricLogoProps, 'src' | 'className' | 'style' | 'preload'>>;
...
const traceShape = (image: HTMLImageElement, raster = RASTER): Shape | null => {
...
// Adição local: contornos já calculados, por resolução + imagem
const shapeCache = new Map<string, Shape>();

const loadShape = (src: string, raster: number): Promise<Shape | null> => {
  const key = raster + '|' + src;
  ...
  return new Promise(resolve => {
    ...
    image.onload = () => {
      let traced: Shape | null;
      try {
        traced = traceShape(image, raster);
      } catch {
        traced = null;
      }
      if (traced) shapeCache.set(key, traced);
      resolve(traced);
    };
...
const ElectricLogo = ({
  ...
  useEffect(() => {
    let alive = true;
    loadShape(src || BOLT, raster).then(traced => {
      if (alive && traced) shapeRef.current = traced;
    });
    return () => {
      alive = false;
    };
  ...
  // Adição local: contorna as próximas imagens uma de cada vez, com uma pausa entre elas
  // para não travar a animação que está rodando
  const preloadKey = preload?.join('\n') ?? '';
  useEffect(() => {
    if (!preloadKey) return undefined;
    let alive = true;
    const queue = preloadKey.split('\n');
    let timer = 0;
    const next = () => {
      const item = queue.shift();
      if (!alive || !item) return;
      loadShape(item, raster).then(() => {
        if (alive) timer = window.setTimeout(next, 60);
      });
    };
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      renderer.dpr = Math.min(window.devicePixelRatio || 1, settingsRef.current?.maxDpr ?? 2, Math.sqrt(PIXEL_BUDGET / (width * height)));
    ...
    const frame = (now: number) => {
      ...
      // Adição local: a troca de forma segue o relógio mesmo abaixo de 20 fps (o dt acima é limitado a 0.05)
      const dtShape = Math.min(0.15, Math.max(0, (now - last) / 1000));
      last = now;
      ...
      if (morph < 1) morph = Math.min(1, morph + dtShape / (pending ? 0.3 : s?.morphDuration ?? 1.6));

      const to = slots[target].shape;
      const from = morph < 1 ? slots[1 - target].shape : null;
      if (to) progress = Math.min(1, progress + dtShape / 1.4);
